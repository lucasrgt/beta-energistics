...
public class BE_BlockFluidRedstoneEmitter extends BlockContainer {
    public BE_BlockFluidRedstoneEmitter(int blockId) {
        super(blockId, Material.iron);
        setHardness(3.5F);
        setResistance(10.0F);
        setStepSound(soundMetalFootstep);
        setBlockName("beFluidRedstoneEmitter");
    }

    @Override
    public TileEntity getBlockEntity() {
        return new BE_TileFluidRedstoneEmitter();
    }
    ...
    public void onNeighborBlockChange(World world, int x, int y, int z, int neighborBlockId) {
        ...
        if (te instanceof BE_TileFluidRedstoneEmitter) {
            BE_TileFluidRedstoneEmitter emitter = (BE_TileFluidRedstoneEmitter) te;
    ...
    public boolean isPoweringTo(IBlockAccess world, int x, int y, int z, int side) {
        TileEntity te = world.getBlockTileEntity(x, y, z);
        if (te instanceof BE_TileFluidRedstoneEmitter) {
            return ((BE_TileFluidRedstoneEmitter) te).getRedstoneOutput() > 0;
        }
        return false;
    }
